...
router.get('/login', function (req, res) {
  // Render the login page and pass the error message as a variable
...
router.get('/pwupdate',function(req,res){
  ...
  res.render('users/pwupdate'); // Render a form for entering the email
});

//routing to set new password when clicking confirmation link in mail
router.get('/confirmpwupdate/:token', async function (req, res) {
  ...
  try {
    ...
    console.error('Error redirecting to set new password',error);
    res.status(500).json({error:'Server error'});
    }
});
//adding userdetails to database
...
router.get('/signupsuccess/:token', async function (req, res) {
  ...
  try {
    ...
    console.error('Error in signupsuccess route:', error);
    res.status(500).json({ error: 'Server error' });
  }
});
...
async function confirmationEmail(email,token) {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: 'EVENTUATE Email Verificatoin',
